...
public class BaseSteps {
  ...
  public static void setHeaders() throws IOException {
    ...
    //  hashMap.put("Authorization", "Bearer " + sessionVariableCalled("token"));
    // }

    if(sessionVariableCalled("token") != null) {
      hashMap.put("Cookie", "token=" + sessionVariableCalled("token"));
    }
  ...
  public static BaseRequestBody createBodyCustom(DataTable dataTable){
    ...
    for (String key : requestData.keySet()) {
      baseRequestBody.addKey(key.replaceAll("\\s+",""), requestData.get(key));
    }
